...
BUFFER_SIZE    equ    16    ; Dosya giris/cikis icin tampon boyu
...
HdrPrcsd       db 0 ; Dosya basligi islenmedi
...
lea	esi, ReadBuffer     ; KaynakIndisi esi <- OkumaTamponu
lea	edi, WriteBuffer    ; HedefIndisi edi <- YazmaTamponu
mov	al, HdrPrcsd        ; Baslik aktarildi mi?
cmp	al, 1
je	HdrPrc              ; aktarilmissa tum tamponu kopyalamaya git
mov	al, [esi]           ; OkumaTamponundan al
mov	[edi], al           ; YazmaTamponuna aktar
inc	esi                 ; Kaynak ve Hedef indislerini ilerlet
inc	edi
mov	al, [esi]           ; OkumaTamponundan al
mov	[edi], al           ; YazmaTamponuna aktar
inc	esi                 ; Kaynak ve Hedef indislerini ilerlet
inc	edi
mov	al, 01h             ; Baslik aktarildi
mov	HdrPrcsd, al        ; Baslik aktarildi bilgisini guncelle
mov	ecx, BUFFER_SIZE - 2 ; Baslik haric geri kalan tampon boyu
jmp	Trnsfr              ; Aktarmaya git

HdrPrc:
mov	ecx, BUFFER_SIZE    ; Baslik aktarildigi icin tum tampon boyu

Trnsfr:
mov	al, [esi]           ; Karakteri YazmaTamponuna aktar
mov	[edi], al
mov	bl, 0               ; Birler sayacýný sýfýrla

CountOnes:
cmp	al, 0               ; 1 kalmadi ise
je	Done                ; sayma sonlanir
shr	al, 1               ; saga 1 bit (elde bayragina) kaydir
jnc	Skip                ; elde bayragi sifirsa 1 sayisini degistirmez
inc	bl                  ; elde bayragi 1 ise sayac artar
Skip:
jmp	CountOnes           ; 1 saymaya devam et

Done:
inc	esi                 ; Unicode da kullanilmayan karakteri gosterir
inc	esi                 ; Sonraki gecerli karakteri gosterir
inc	edi                 ; Berger kodu yazilacak yeri gosterir
not	bl                  ; 1 sayisinin tumleyeni
mov	[edi], bl           ; Berger kodunu YazmaTamponuna aktar
inc	edi                 ; Sonraki gecerli karakteri gosterir
dec	ecx                 ; Tampon aktarimi bitene kadar don
jnz	Trnsfr
...
push    offset WriteBuffer  ; tampon
